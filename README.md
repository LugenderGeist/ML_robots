...
F1-score на тестовой выборке вырос с 57% до 81.48%. Модель стала значительно стабильнее — разброс F1-score по фолдам уменьшился с 11.11% до 9.06%. Функция активации tanh показала лучшие результаты на нормированных данных, что объясняется ее симметричностью относительно нуля. Уменьшение архитектуры до (50,25) указывает на то, что нормированные данные требуют меньшей емкости сети.
...
StandardScaler показал наилучший результат на тестовой выборке с F1-score 84.62%, что является максимальным значением среди всех экспериментов. Модель также характеризуется наименьшим разбросом метрик по фолдам (6.70% для F1-score). Лучшая архитектура стала глубже — (80,40,20), а функция активации — relu. Это объясняется тем, что стандартизация приводит данные к распределению, близкому к нормальному, с которым relu работает эффективно.
